...
const GdsResult = ({ user, evaluation, data }: {
    ...
    const chartData = [
        ...
        ...gdsQuestions.map((q, index) => [
            `Q${index + 1}`,
            (parseInt(data[q.field as keyof Gds]) === 1 ? 1 : -1) * (q.score === 'Affirmative' ? 1 : -1),
            `${boldify(t(q.question))}\n${boldify('Valor:')} ${['Não', 'Sim'][q.score === 'Affirmative' ? data[q.field] : +!+data[q.field]]}`
        ])
    ];
                    ...
                    <TableBody>
                        {gdsQuestions.map((item) => (
                            <TableRow className="h-20" key={item.index}>
                                <TableCell className="w-1/2 text-md border-r-2">{t(item.question)}</TableCell>
                                <TableCell
                                    className="w-1/4 text-white text-center text-md"
                                    style={{ backgroundColor: ['var(--primary-background)', 'var(--primary)'][item.score === 'Affirmative' ? data[item.field] : +!+data[item.field]] }}
                                >
                                    {['', 'X'][item.score === 'Affirmative' ? data[item.field] : +!+data[item.field]]}
                                </TableCell>
                                <TableCell
                                    className="w-1/4 text-white text-center text-md"
                                    style={{ backgroundColor: ['var(--primary)', 'var(--primary-background)'][item.score === 'Affirmative' ? data[item.field] : +!+data[item.field]] }}
                                >
                                    {['X', ''][item.score === 'Affirmative' ? data[item.field] : +!+data[item.field]]}
                                </TableCell>
                            </TableRow>
